...
# Load live music venues with websites
venues_df = pl.scan_csv(os.path.join(OUTPUT_FOLDER, "live_music.csv")).collect()
venues = venues_df.filter(
    (pl.col("Website") != "N/A") &
    (pl.col("Website") != "")
).select("Name", "Address", "Website").to_dicts()

print(f"Scraping {len(venues)} live music venue(s) via OpenAI (concurrent).\n")
...
if all_events:
    ...
    df.write_csv(os.path.join(OUTPUT_FOLDER, "live_music_events.csv"))
    print(f"Saved to output_folder/live_music_events.csv")
